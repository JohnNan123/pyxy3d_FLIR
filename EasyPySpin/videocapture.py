...
class VideoCapture:
    ...
    def retrieve(self) -> Tuple[bool, Union[np.ndarray, None]]:
        ...
        if hasattr(self, "_pyspin_image"):
            # image_array = self._pyspin_image.GetNDArray() #comment out by JN, original code
            # return True, image_array
            # following line are add by JN
            image_array = self._pyspin_image.GetNDArray()
            RGB_array = cv2.cvtColor(image_array, cv2.COLOR_BayerRG2RGB)
